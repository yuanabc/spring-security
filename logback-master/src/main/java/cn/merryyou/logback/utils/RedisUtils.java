...
public class RedisUtils implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2745709041310675989L;
    ...
    public void setExpire(final byte[] key, final byte[] value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            log.debug("[redis] save cache  url:{} ======== cache time {} ms", key, time);
            return 1L;
        });
    }
    ...
    public void setExpire(final String[] keys, final String[] values, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                log.debug("[redis] save cache  url:{} ======== cache time {} ms", keys[i], time);
            }
            return 1L;
        });
    }
    ...
    public void set(final String[] keys, final String[] values) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                byte[] bValues = serializer.serialize(values[i]);
                connection.set(bKeys, bValues);
                log.debug("[redis] save cache  url:{}", keys[i]);
            }
            return 1L;
        });
    }
    ...
    public void set(final String key, final String value) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            byte[] values = serializer.serialize(value);
            connection.set(keys, values);
            log.debug("[redis] save cache  url:{}", key);
            return 1L;
        });
    }
    ...
    public byte[] get(final byte[] key) {
        ...
        log.debug("[redis]get cache  url:{} ", key);
        return result;
    }
    ...
    public Map<String, String> getKeysValues(final String keyPatten) {
        log.debug("[redis]  getValues()  patten={} ", keyPatten);
    ...
    public void putHashValue(String key, String hashKey, String hashValue) {
        log.debug("[redis]  putHashValue()  key={},hashKey={},hashValue={} ", key, hashKey, hashValue);
    ...
    public Object getHashValues(String key, String hashKey) {
        log.debug("[redis]  getHashValues()  key={},hashKey={}", key, hashKey);
    ...
    public void delHashValues(String key, Object... hashKeys) {
        log.debug("[redis]  delHashValues()  key={}", key);
    ...
    public Map<String, String> getHashValue(String key) {
        log.debug("[redis]  getHashValue()  key={}", key);
