...
public class DefaultRedisOperateService implements RedisOperateService {
    ...
    public Long increment(String key, Long step, Integer time) {
        ...
        Long sequence = stringRedisTemplate.opsForValue().increment(CacheKey.timeSequence + key, step);
        stringRedisTemplate.expire(CacheKey.timeSequence + key, time, TimeUnit.SECONDS);
        return sequence;
    }
}
