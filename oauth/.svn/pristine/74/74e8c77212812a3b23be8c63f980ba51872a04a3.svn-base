...
public class DefaultUserService implements UserService {
    ...
    @Override
    public Optional<PageResult<List<UserDO>>> querySalesWithPage(PageParam<SalesQueryPageParamDTO> pageParam) {
//        // 设置机构范围
...
//        return queryWithPage(pageParam);
    ...
    public Optional<Long> queryMaxCode(String channelCode, CodeTypeEnum codeTypeEnum, long min) {
        ...
        return Optional.ofNullable(customUserMapper.queryMaxCode(channelCode, userType, min));
    }
    ...
    public Optional<PageResult<List<UserDO>>> queryUserLinkCompanyAndLinkRoleWithPage(PageParam<UserQueryPageParamDTO> pageParam) {
        ...
        if (!ids.isEmpty()) {
            ...
            result.forEach(UserDO::setBelongCompanyAndManagerCompany);
            result.sort(Comparator.comparingLong(UserDO::getCreateTime));
        } else {
