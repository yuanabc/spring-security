        ...
        filterTreeNodes.ifPresent(treeNodes -> {
            treeNodes.forEach(treeNode -> {
                ...
                while (limit >= 0) {
                    ...
                    List<TreeNode> filterChildrenTreeNode = Optional.ofNullable(parent.getChildren()).orElseGet(ArrayList::new)
                            .stream()
                            .filter(children -> StringUtils.equals(children.getKey(), tempTreeNode.getKey()))
                            .collect(Collectors.toList());
                    parent.setChildren(filterChildrenTreeNode);
